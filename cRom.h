...
typedef struct
{
	...
	UINT8*		Dyna4Cols;  // UINT8[nF*MAX_DYNA_SETS_PER_FRAME*noC] Color sets used to fill the dynamic content	UINT8*		FrameSprites; // UINT8[nF*MAX_SPRITES_PER_FRAME] Sprite numbers to look for in this frame max=MAX_SPRITES_PER_FRAME 255 if no sprite
	...
	UINT8*		ColorRotations; // UINT8[3*MAX_COLOR_ROTATION*nF] 3 bytes per rotation: 1- first color (255 if no rotation); 2- last color; 3- number of 10ms between two rotations
...
typedef struct
{
	...
	char		SaveDir[260]; // char[260] VPinMAME directory (must contain the final '\')
